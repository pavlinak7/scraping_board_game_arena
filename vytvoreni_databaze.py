...
load_csv_to_db("ukoncene_hry", '/home/pavlina/Dokumenty/IT/scraping_bga_nejnovejsi/ukoncene_hry/zaklad/ukoncene_hry.csv')
...
def load_file_to_db(table, csv_file_path):
    ...
    for index, row in data.iterrows():
        cursor.execute(
            ...
            (row['url'], row['Tournament'], row['Tournament url'], row['Game Option Value'], row['Tournament url2'])
        )
    conn.commit()
    cursor.close()
    conn.close()
...
load_file_to_db("typ_hry", '/home/pavlina/Dokumenty/IT/scraping_bga_nejnovejsi/ukoncene_hry/zaklad/tournaments.csv')
#####################################################################################################################
def create_table_typ_hry(table):
    # Connect to the 'bga' database
    conn = psycopg2.connect(dbname='bga', **db_config)
...
load_file_to_db("umisteni_na_turnajich", '/home/pavlina/Dokumenty/IT/scraping_bga_nejnovejsi/ukoncene_hry/zaklad/umisteni_na_turnajich.csv')